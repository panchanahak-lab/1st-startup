  ...
  const ClassicTemplate = () => (
    <div className="p-[10mm] md:p-[20mm] text-slate-900 flex flex-col font-sans text-[10pt] md:text-[11pt] bg-white h-full min-h-[297mm]">
      <header className="border-b-2 border-slate-900 pb-4 md:pb-6 mb-6 md:mb-8 text-center">
        ...
        <div className="flex flex-wrap justify-center gap-x-4 md:gap-x-6 gap-y-1 text-[10px] md:text-sm text-slate-500 mt-3 md:mt-4 font-medium">
      ...
      <section className="mb-6 md:mb-8">
        <h3 className="text-[9pt] md:text-[10pt] font-black uppercase border-b border-slate-900 pb-1 mb-4 md:mb-6 tracking-widest">Experience</h3>
        {data.experience?.map(exp => (
          <div key={exp.id} className="mb-6 md:mb-8">
            <div className="flex flex-col md:flex-row md:justify-between font-bold text-xs md:text-sm mb-1">
              <span className="uppercase text-slate-900">{exp.role}</span>
              <span className="text-slate-400 md:text-slate-500">{exp.date}</span>
            </div>
            <div className="flex justify-between items-center mb-2 md:mb-3">
              <span className="text-xs md:text-sm text-brand-600 font-bold uppercase tracking-wider">{exp.company}</span>
              <span className="text-[8px] md:text-[10px] text-slate-400 font-bold uppercase tracking-widest">{exp.location}</span>
            </div>
            <ul className="list-disc pl-4 md:pl-5 space-y-1.5 md:space-y-2">
              {exp.bullets.filter(b => b.trim()).map((b, i) => (
                <li key={i} className="text-[11px] md:text-sm text-slate-700 leading-snug">{b}</li>
              ))}
            </ul>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 mt-auto">
        <section>
          <h3 className="text-[9pt] md:text-[10pt] font-black uppercase border-b border-slate-900 pb-1 mb-3 md:mb-4 tracking-widest">Education</h3>
          {data.education?.map(edu => (
            <div key={edu.id} className="mb-3">
              <p className="font-bold text-xs md:text-sm">{edu.degree}</p>
              <p className="text-[11px] md:text-sm text-slate-600">{edu.school} • {edu.year}</p>
            </div>
          ))}
        </section>
        {data.certifications && data.certifications.length > 0 && (
          <section className="mb-8">
            <h3 className="text-[9pt] md:text-[10pt] font-black uppercase border-b border-slate-900 pb-1 mb-3 md:mb-4 tracking-widest">Certifications</h3>
            {data.certifications.map(cert => (
              <div key={cert.id} className="mb-2">
                <p className="font-bold text-xs md:text-sm">{cert.name}</p>
                <p className="text-[11px] md:text-sm text-slate-600">{cert.issuer} • {cert.date}</p>
              </div>
            ))}
          </section>
        )}
        <section>
          <h3 className="text-[9pt] md:text-[10pt] font-black uppercase border-b border-slate-900 pb-1 mb-3 md:mb-4 tracking-widest">Languages</h3>
          <div className="space-y-1">
            {data.languages.map(l => (
              <p key={l.id} className="text-[11px] md:text-sm font-medium text-slate-700">{l.name} — <span className="italic text-slate-400">{l.level}</span></p>
            ))}
          </div>
        </section>
      </div>
    ...
    <div className="flex flex-col md:flex-row bg-white h-full min-h-[297mm] font-sans">
      <div className="w-full md:w-[32%] bg-navy-950 p-6 md:p-10 text-white">
        <div className="mb-8 md:mb-12">
          ...
          <h2 className="text-[9px] md:text-[10px] font-bold text-brand-400 uppercase tracking-[0.2em] md:tracking-[0.3em]">{data.targetRole || 'ROLE'}</h2>
        </div>
        <div className="space-y-8 md:space-y-12">
          <section>
            <h3 className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] md:tracking-[0.3em] text-slate-500 mb-4 md:mb-6 border-b border-white/5 pb-2">Technical</h3>
            <div className="flex flex-wrap gap-1.5 md:gap-2">
              {data.hardSkills.split(',').map((s, i) => (
                <span key={i} className="text-[8px] md:text-[9px] bg-white/5 border border-white/10 px-2 py-1 md:px-2.5 md:py-1.5 rounded-md font-bold">{s.trim()}</span>
              ))}
            </div>
          </section>
          <section>
            <h3 className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] md:tracking-[0.3em] text-slate-500 mb-4 md:mb-6 border-b border-white/5 pb-2">Languages</h3>
            <div className="grid grid-cols-2 md:grid-cols-1 gap-4">
              {data.languages.map(l => (
                <div key={l.id}>
                  <p className="text-[11px] md:text-xs font-bold text-white">{l.name}</p>
                  <p className="text-[8px] md:text-[9px] text-slate-400 uppercase tracking-widest mt-0.5">{l.level}</p>
                </div>
              ))}
            </div>
          </section>
      ...
      <div className="w-full md:w-[68%] p-8 md:p-16">
        <section className="mb-12 md:mb-16">
          <h3 className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.3em] md:tracking-[0.4em] text-slate-300 mb-6 md:mb-10 flex items-center gap-3 md:gap-4">
            <span className="w-6 md:w-8 h-[2px] bg-brand-500"></span> Experience
          </h3>
          <div className="space-y-8 md:space-y-12">
            {data.experience?.map(exp => (
              <div key={exp.id} className="relative pl-6 md:pl-10">
                <div className="absolute left-0 top-1 w-2 md:w-3 h-2 md:h-3 bg-brand-500 rounded-full border-2 md:border-4 border-white shadow-lg z-10"></div>
                ...
                <div className="flex flex-col md:flex-row md:justify-between md:items-baseline mb-2">
                  <span className="font-black text-base md:text-lg text-navy-900 tracking-tight">{exp.role}</span>
                  <span className="text-[9px] md:text-[10px] text-brand-500 font-black uppercase tracking-widest mt-0.5 md:mt-0">{exp.date}</span>
                </div>
                <div className="flex flex-wrap items-center gap-2 md:gap-3 mb-3 md:mb-4">
                  <span className="text-[10px] md:text-xs text-slate-400 font-bold uppercase tracking-wider">{exp.company}</span>
                  <span className="hidden md:block w-1 h-1 bg-slate-200 rounded-full"></span>
                  <span className="text-[10px] md:text-xs text-slate-400 font-bold uppercase tracking-wider">{exp.location}</span>
                </div>
                <ul className="space-y-2 md:space-y-3">
                  {exp.bullets.filter(b => b.trim()).map((b, i) => (
                    <li key={i} className="text-xs md:text-[13px] text-slate-600 leading-relaxed relative flex gap-2 md:gap-3">
                      <span className="text-brand-500 mt-1 md:mt-1.5">•</span> {b}
                    </li>
                  ))}
                </ul>
              </div>
  ...
  const CreativeTemplate = () => (
    <div className="bg-white h-full min-h-[297mm] font-sans text-slate-800">
      <header className="bg-gradient-to-br from-navy-900 to-navy-950 p-8 md:p-16 text-white text-center relative overflow-hidden">
        ...
        <h2 className="text-sm md:text-xl font-bold text-brand-400 uppercase tracking-[0.3em] md:tracking-[0.5em] relative z-10">{data.targetRole || 'TARGET ROLE'}</h2>
      </header>
      <div className="flex flex-col md:grid md:grid-cols-[250px_1fr] lg:grid-cols-[320px_1fr]">
        <aside className="bg-slate-50/50 p-8 md:p-12 border-b md:border-b-0 md:border-r border-slate-100">
          <section className="mb-8 md:mb-12">
            <h3 className="text-[10px] md:text-xs font-black text-navy-900 uppercase tracking-widest mb-4 md:mb-6 border-b-2 border-brand-500/20 pb-2">Technical</h3>
            <div className="flex flex-wrap gap-1.5 md:gap-2">
              {data.hardSkills.split(',').map((s, i) => (
          ...
          <section>
            ...
            <div className="space-y-3 md:space-y-4">
              {data.languages.map(l => (
                <div key={l.id} className="flex justify-between items-center text-[10px] md:text-[11px] font-bold">
              ...
              <div key={exp.id} className="relative group">
                <div className="mb-4 md:mb-6">
                  <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-1">
                    <h4 className="text-lg md:text-xl font-black text-navy-900 uppercase tracking-tight group-hover:text-brand-500 transition-colors">{exp.role}</h4>
                    <span className="text-[9px] md:text-[10px] font-black text-slate-300 uppercase tracking-widest mt-1 md:mt-0">{exp.date}</span>
                  </div>
                  <div className="flex items-center gap-2 md:gap-3">
                    <p className="text-[10px] md:text-xs font-black text-brand-600 uppercase tracking-widest">{exp.company}</p>
                    <span className="w-1 h-1 bg-slate-200 rounded-full"></span>
                    <p className="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-widest">{exp.location}</p>
                  </div>
                </div>
                <ul className="space-y-2 md:space-y-3 mt-4 md:mt-6 border-l-2 border-slate-50 pl-4 md:pl-6">
                  {exp.bullets.filter(b => b.trim()).map((b, i) => (
                    <li key={i} className="text-[12px] md:text-[13px] text-slate-500 leading-relaxed font-medium">{b}</li>
                  ))}
                </ul>
              </div>
            ))}
  ...
  const AcademicTemplate = () => (
    <div className="p-[10mm] md:p-[25mm] bg-white h-full min-h-[297mm] font-serif text-slate-900">
      ...
      <section className="mb-8 md:mb-12">
        ...
        {data.experience?.map(exp => (
          <div key={exp.id} className="mb-8 md:mb-10">
            <div className="flex flex-col md:flex-row md:justify-between font-bold text-base md:text-lg mb-1">
              <span>{exp.role}</span>
              <span className="text-slate-400 font-normal italic text-[11px] md:text-sm mt-0.5 md:mt-0">{exp.date}</span>
            </div>
            <div className="flex justify-between items-center mb-3 md:mb-4 text-[11px] md:text-sm text-slate-600">
              <span className="italic">{exp.company}</span>
              <span className="uppercase tracking-widest text-[8px] md:text-[10px]">{exp.location}</span>
            </div>
            <ul className="list-disc pl-6 md:pl-8 space-y-1.5 md:space-y-2">
              {exp.bullets.filter(b => b.trim()).map((b, i) => (
                <li key={i} className="text-[10pt] md:text-[11pt] leading-relaxed text-slate-700">{b}</li>
              ))}
            </ul>
          </div>
        ))}
      ...
      {data.certifications && data.certifications.length > 0 && (
        <section className="mb-8 md:mb-12">
          <h3 className="text-sm md:text-base font-bold uppercase tracking-[0.2em] md:tracking-[0.3em] text-slate-300 mb-6 md:mb-8 border-b border-slate-100 pb-2">Certifications</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {data.certifications.map(cert => (
              <div key={cert.id}>
                <p className="text-[10pt] md:text-[11pt] font-bold text-slate-900">{cert.name}</p>
                <p className="text-[9pt] md:text-[10pt] italic text-slate-600">{cert.issuer} ({cert.date})</p>
              </div>
            ))}
          </div>
        </section>
      ...
      <section>
        <h3 className="text-sm md:text-base font-bold uppercase tracking-[0.2em] md:tracking-[0.3em] text-slate-300 mb-6 md:mb-8 border-b border-slate-100 pb-2">Languages</h3>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-6 md:gap-8">
          {data.languages.map(l => (
            <div key={l.id}>
              <p className="text-[10pt] md:text-[11pt] font-bold text-slate-900">{l.name}</p>
              <p className="text-[9pt] md:text-[10pt] italic text-slate-500">{l.level}</p>
            </div>
          ))}
        </div>
      </section>
