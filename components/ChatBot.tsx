  ...
  const handleSend = async (overrideText?: string) => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: [
          { role: 'user', parts: [{ text: `System: You are the NextStep Career Architect. You are helpful, strategic, and professional. 
          Context: NextStep Resume provides an ATS Audit (Scanner), a Premium CV Builder, and a Neural Interview Simulator.
          
          User Query: ${userMessage.text}` }] }
        ],
      });

      const aiText = response.text || "I encountered a synchronization error. Please re-state your query.";
  ...
  return (
    <div id="chatbot-trigger" className="fixed bottom-6 right-6 md:bottom-8 md:right-8 z-[9999] print:hidden">
      ...
      <button
        ...
        className={`w-14 h-14 md:w-16 md:h-16 rounded-[1.5rem] md:rounded-[2rem] shadow-2xl flex items-center justify-center text-white transition-all duration-500 transform hover:scale-110 active:scale-95 group relative overflow-hidden ${
          isOpen ? 'bg-navy-900 rotate-90 shadow-navy-900/20' : 'bg-brand-500 shadow-brand-500/40'
        }`}
          ...
          <div className="flex-1 overflow-y-auto p-5 md:p-8 space-y-6 bg-slate-50 dark:bg-navy-950 custom-scrollbar relative">
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[90%] md:max-w-[85%] p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] text-sm shadow-sm relative ${
                  msg.role === 'user' 
          ...
          <div className="p-5 md:p-8 bg-white dark:bg-navy-950 border-t border-slate-200 dark:border-white/10 space-y-5 md:space-y-6 shrink-0 pb-8 md:pb-8">
            ...
            <div className="relative flex items-center group">
              ...
              <button 
                ...
                className={`absolute right-2 w-10 h-10 md:w-12 md:h-12 rounded-lg md:rounded-xl flex items-center justify-center transition-all ${
                  input.trim() && !isTyping ? 'bg-brand-500 text-white shadow-lg' : 'bg-slate-200 dark:bg-white/5 text-slate-400'
                }`}
              >
