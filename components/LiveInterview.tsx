...
const LiveInterview: React.FC = () => {
  ...
  const askCurrentQuestion = async () => {
    ...
    // Transition to ASK_QUESTION or ASK_FOLLOW_UP
    if (state === 'PROCESSING') {
      transitionTo('ASK_FOLLOW_UP');
    } else if (state !== 'ASK_QUESTION' && state !== 'INITIALIZING') {
      transitionTo('ASK_QUESTION');
    }
...
Keep it encouraging but honest.`;
  ...
  const isActiveInterview = ['ASK_QUESTION', 'LISTENING', 'PROCESSING', 'ASK_FOLLOW_UP'].includes(state);
          ...
          {isActiveInterview && (
            <div className="grid lg:grid-cols-[1fr_400px] gap-12 animate-fade-in items-start">
              <div className="space-y-8">
                ...
                <div className="flex gap-4">
                  {state === 'LISTENING' && (
                    <button onClick={skipQuestion} className="flex-1 py-4 bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl font-bold transition-all flex items-center justify-center gap-2">
              ...
              <div className="grid md:grid-cols-5 gap-4">
                {feedback.ruleScore.dimensions.map((dim, i) => (
                  <div key={i} className="bg-slate-50 p-4 rounded-2xl text-center">
                    <div className={`text-3xl font-black mb-2 ${dim.score >= 4 ? 'text-green-600' : dim.score >= 3 ? 'text-yellow-600' : 'text-red-600'}`}>
                      {dim.score}
                    </div>
