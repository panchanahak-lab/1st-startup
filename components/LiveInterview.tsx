...
const PERSONAS = {
  mentor: { name: 'Supportive Mentor', icon: 'fa-heart', instruction: 'be encouraging, focus on potential, and offer gentle guidance.' },
  recruiter: { name: 'Professional Recruiter', icon: 'fa-user-tie', instruction: 'be objective, ask standard industry questions, and focus on culture fit.' },
  stress: { name: 'High-Pressure Executive', icon: 'fa-bolt', instruction: 'be aggressive, challenge their numbers, press for specific metrics, and act skeptical.' }
};
        ...
        try {
          ...
          const response = await ai.models.generateContent({
            ...
            contents: { parts: [
              ...
              { text: "Extract the core experience, top 5 technical skills, and 3 key achievements from this resume. Keep it brief and bulleted." }
            ] }
          });
          setResumeContext(response.text || "Resume content unavailable.");
          setResumeFile(file);
          setStage('setup');
        } catch (err) {
          setErrorDetail({
          ...
          setResumeFile(file); // Keep file even if parse fails for simple upload tracking
          setStage('setup');
        }
      };
  ...
  const stopAndFeedback = async () => {
    ...
    try {
      ...
      const res = await ai.models.generateContent({
        ...
        contents: `Evaluate this interview for a ${jobRole} role in ${language} with a ${persona} interviewer.
        
        JSON SCHEMA:
        { "score": number, "summary": string, "strengths": string[], "weaknesses": string[], "suggestions": string[], "idealResponseTip": string }
        ...
        config: {
          ...
          responseSchema: {
            type: Type.OBJECT,
            properties: { 
              score: { type: Type.NUMBER }, summary: { type: Type.STRING }, 
              strengths: { type: Type.ARRAY, items: { type: Type.STRING } }, 
              weaknesses: { type: Type.ARRAY, items: { type: Type.STRING } }, 
              suggestions: { type: Type.ARRAY, items: { type: Type.STRING } },
              idealResponseTip: { type: Type.STRING }
            },
      ...
      <div className="max-w-5xl mx-auto px-4 relative z-10">
        <div className="text-center mb-16 animate-fade-in">
          <div className="inline-flex items-center gap-3 bg-brand-500/10 text-brand-400 px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.3em] mb-8 border border-brand-500/20 shadow-lg shadow-brand-500/5">
            ...
            Neural Career Simulator
          </div>
          <h2 className="text-5xl md:text-7xl font-extrabold mb-6 tracking-tighter leading-tight">Elite Interview <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-400 to-blue-500">Mastery</span></h2>
          <p className="text-slate-400 max-w-2xl mx-auto text-lg">Don't just practice. Simulate the pressure, the specific questions, and the native tone of global recruiters.</p>
        </div>
        ...
        <div className={`glass dark:bg-navy-900/40 rounded-[3rem] border border-white/5 backdrop-blur-2xl p-8 md:p-16 shadow-2xl transition-all duration-700 relative overflow-hidden ${stage === 'feedback' ? 'bg-white !text-navy-950' : ''}`}>
            ...
            <div className="max-w-2xl mx-auto space-y-10 animate-fade-in-up">
              <div className="grid md:grid-cols-2 gap-8">
                <div className="space-y-6">
                  <div>
                    ...
                    <input value={jobRole} onChange={e => setJobRole(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl p-5 outline-none focus:ring-2 focus:ring-brand-500 transition-all text-white placeholder-slate-600 font-bold" placeholder="e.g. Senior Product Lead" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-black uppercase text-slate-500 mb-4 tracking-widest">Dialect/Language</label>
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase text-slate-500 mb-4 tracking-widest">Interviewer Difficulty</label>
                  <div className="grid gap-3">
                    {Object.entries(PERSONAS).map(([key, p]) => (
                      <button 
                        key={key} 
                        onClick={() => setPersona(key as any)}
                        className={`p-4 rounded-2xl border text-left transition-all flex items-center gap-4 group ${persona === key ? 'border-brand-500 bg-brand-500/10 text-white' : 'border-white/5 bg-white/5 text-slate-500 hover:border-white/20'}`}
                      >
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors ${persona === key ? 'bg-brand-500 text-white' : 'bg-white/10 group-hover:bg-white/20'}`}>
              ...
              <div className="bg-navy-950 p-8 rounded-[2rem] border border-white/5 group/file relative overflow-hidden">
                <div className="flex items-center justify-between mb-4">
                  <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Intelligence Layer</h4>
                ...
                >
                  ...
                  <p className="text-sm font-bold">{resumeFile ? resumeFile.name : "Upload Resume to enable Intelligence Analysis"}</p>
                  <p className="text-[10px] mt-2 opacity-40 font-bold uppercase tracking-widest">PDF or DOCX preferred</p>
                </div>
              </div>
              ...
              <div className="text-center">
                ...
                <p className="mt-4 text-[10px] text-slate-500 font-bold uppercase tracking-widest flex items-center justify-center gap-2">
                  <i className="fas fa-microphone text-brand-500"></i> Secure end-to-end audio sync
                </p>
              </div>
            </div>
          )}
          ...
          {(stage === 'parsing' || stage === 'initializing') && (
            <div className="h-[500px] flex flex-col items-center justify-center space-y-10">
               ...
               <div className="text-center">
                 <h3 className="text-3xl font-black uppercase mb-4 tracking-tighter">{stage === 'parsing' ? 'Parsing Intelligence' : 'Syncing Live Agent'}</h3>
                 <p className="text-slate-500 max-w-sm mx-auto font-medium">Injecting resume context and calibrating native audio streams...</p>
            ...
            <div className="grid lg:grid-cols-[1fr_400px] gap-12 animate-fade-in items-start">
              <div className="space-y-8">
                <div className="bg-navy-950 rounded-[3rem] border border-white/10 overflow-hidden shadow-2xl relative group">
                  ...
                  <div className="absolute top-6 left-6 flex items-center gap-4">
                     ...
                     <span className="text-xs font-black uppercase tracking-widest text-white/50">Neural Link Active</span>
                  </div>
                </div>
                ...
                <div className="bg-white/5 p-12 rounded-[3rem] border border-white/5 min-h-[250px] flex flex-col justify-center text-center relative group">
                  ...
                  <p className="text-[10px] font-black text-brand-400 mb-6 uppercase tracking-[0.5em]">Real-time Transcription</p>
                  <p className="text-2xl md:text-3xl font-bold leading-tight italic text-slate-100 px-4">
                    {liveAiText ? `"${liveAiText}"` : liveUserText ? "Candidate speaking..." : "Interviewer is considering your answer..."}
                  </p>
                </div>
              ...
              <div className="bg-navy-950/80 p-8 rounded-[3rem] border border-white/10 flex flex-col h-[700px] shadow-2xl">
                <div className="flex items-center justify-between mb-8 pb-4 border-b border-white/5">
                  <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">Neural Log</h3>
                  <div className="px-3 py-1 bg-brand-500/10 text-brand-500 rounded-full text-[8px] font-black uppercase">Persona: {persona}</div>
                </div>
                ...
                <div className="flex-grow overflow-y-auto custom-scrollbar space-y-8 pr-4">
                  {transcripts.length === 0 && !liveUserText && (
                    <div className="h-full flex flex-col items-center justify-center text-center opacity-20">
                       ...
                       <p className="text-xs font-black uppercase tracking-widest">Awaiting Audio Pulse</p>
                    </div>
                  )}
                  {transcripts.map((t, i) => (
          ...
          {stage === 'processing_feedback' && (
            <div className="h-[500px] flex flex-col items-center justify-center space-y-10">
               ...
               <div className="text-center">
                 <h3 className="text-4xl font-black mb-4 tracking-tighter">Diagnostic Report</h3>
                 <p className="text-slate-500 max-w-sm mx-auto font-medium">Aggregating linguistics, tone match, and contextual relevance scores...</p>
               </div>
            </div>
          )}
          ...
          {stage === 'feedback' && feedback && (
             <div className="animate-fade-in-up space-y-16">
               <div className="text-center">
                 ...
                 <h2 className="text-4xl font-black uppercase tracking-tighter mb-6">Simulation Analytics</h2>
               ...
               <div className="grid md:grid-cols-2 gap-12">
                 <div className="bg-green-500/5 p-10 rounded-[3rem] border border-green-500/10 shadow-sm">
                   <h4 className="text-green-600 font-black text-xs mb-8 flex items-center gap-4 uppercase tracking-widest">
                     <i className="fas fa-shield-check text-xl"></i> High Impact Signals
                   </h4>
                 ...
                 <div className="bg-red-500/5 p-10 rounded-[3rem] border border-red-500/10 shadow-sm">
                   <h4 className="text-red-600 font-black text-xs mb-8 flex items-center gap-4 uppercase tracking-widest">
                     <i className="fas fa-triangle-exclamation text-xl"></i> Optimization Gaps
                   </h4>
                   <ul className="space-y-6">
               ...
               <div className="bg-brand-500/5 p-12 rounded-[3.5rem] border border-brand-500/10 shadow-lg relative overflow-hidden">
                 ...
                 <h4 className="text-brand-600 font-black text-xs mb-10 flex items-center gap-4 uppercase tracking-widest">
                    <i className="fas fa-sparkles text-xl"></i> Executive Mastery Tip
                 </h4>
               ...
               <div className="flex flex-col sm:flex-row gap-6 pt-10 print:hidden">
                 <button onClick={() => setStage('setup')} className="flex-1 py-6 bg-navy-900 text-white hover:bg-navy-800 rounded-2xl font-black text-xl shadow-2xl transition-all hover:scale-[1.02] active:scale-95">Restart Simulation</button>
                 <button onClick={() => window.print()} className="flex-1 py-6 bg-brand-500 text-white hover:bg-brand-600 rounded-2xl font-black text-xl shadow-2xl transition-all hover:scale-[1.02] active:scale-95">Export Diagnostic</button>
               </div>
             </div>
          )}
        </div>
